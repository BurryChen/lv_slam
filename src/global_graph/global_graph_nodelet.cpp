...
private:
  ...
  bool flush_imu_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      try {
        ...
        if(enable_imu_orientation)tf_listener.transformQuaternion(base_frame_id, quat_imu, quat_base);
  ...
  void save_pose(std::string directory) {
	...
	if(j==seq0)pose_new=kf_pose*pose_s2k;
	else pose_new=kf_pose*pose_s2k*d_pose_odom;
