...
#for seq in 00 02 03 04 05 06 07 08 09 10  #using s2k
for seq in 08  #highway using s2k_floor
do

file=${1}/data/KITTI_${seq}_odom.txt
echo $seq $file
gnome-terminal -x bash -c "echo $seq;roslaunch LVO ndt_odom_kitti.launch odom_file:=$file &sleep 10s;rosbag play --clock /media/whu/HD_CHEN_2T/02data/KITTI_odometry/velobag/velo_${seq}.bag -r 0.3;echo $seq over&&sleep 20s;exit"
i=10#$seq
time=`expr 60 + ${durations[i]} \* 10 / 3`
echo $time s
sleep $time
wait
