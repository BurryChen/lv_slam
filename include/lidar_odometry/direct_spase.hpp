...
// 直接稀疏法估计位姿
bool reg_direct_sparse ( const pcl::PointCloud<PointT>::Ptr& source_cloud,string source_img,string target_img,
			 Eigen::Matrix4d tf_velo2cam2,Eigen::Matrix3d K,Eigen::Matrix4d pose_velo )
{
    cv::Mat color,gray;
    vector<Measurement> measurements;
    
    color= cv::imread ( source_img ,1);
    if ( color.data==nullptr )
      return 0; 
    cv::cvtColor ( color, gray, cv::COLOR_BGR2GRAY );
    measurements=project_cloud2img(source_cloud,gray,tf_velo2cam2,K);

    color= cv::imread ( target_img ,1);
    if ( color.data==nullptr )
      return 0; 
    cv::cvtColor ( color, gray, cv::COLOR_BGR2GRAY );
    
    Eigen::Isometry3d pose_cam2(tf_velo2cam2*pose_velo*tf_velo2cam2.inverse());
    poseEstimationDirect ( measurements, &gray, K, pose_cam2 );
    pose_velo=tf_velo2cam2.inverse()*pose_cam2.matrix()*tf_velo2cam2;
    return 1;
};
